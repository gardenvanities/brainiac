...
- Teste: verificação UI — modelo hardcoded e botão `+` ausentes; banner aparece com o nome real do documento ativo (`resolveInlineTitle`) e some por completo sem doc (sem faixa vazia)
- Pronto quando: verificado + `bun run check` verde

## Tarefa 7: Separação única e mais leve
...
## Tarefa 9: Verificação final (DoD + verification)
